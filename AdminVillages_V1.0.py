...
target_district = arcpy.GetParameterAsText(3)
...
# ------------------- Village Layout Logic -------------------
if layout_level == "village":
    district_field = "DistrictName"
    taluka_field = "TalukaName"
    village_field = "villagenamecode"
    village_text_field = "VillageName"
    exported_count = 0  

    # Track totals across all districts
    village_total = 0
    village_skipped = 0
    village_exported = 0
    ...
    district_names = [name for name in district_names if name.lower() == target_district.lower()]
    ...
    for district_index, district_name in enumerate(district_names, 1):
        ...
        for taluka_index, taluka_name in enumerate(taluka_names, 1):
            ...
            district_frame = next((mf for mf in layout_template.listElements("MAPFRAME_ELEMENT") if "district frame" in mf.name.lower()), None)
            taluka_frame = next((mf for mf in layout_template.listElements("MAPFRAME_ELEMENT") if "taluka frame" in mf.name.lower()), None)
            village_frame = next((mf for mf in layout_template.listElements("MAPFRAME_ELEMENT") if "village frame" in mf.name.lower()), None)

            if not district_frame or not taluka_frame or not village_frame:
                arcpy.AddError("❌ Required map frames missing in layout.")
                continue

            district_map = district_frame.map
            taluka_map = taluka_frame.map
            village_map = village_frame.map

            district_layer = next((lyr for lyr in district_map.listLayers() if lyr.name.upper() == "DISTRICT"), None)
            taluka_layer = next((lyr for lyr in taluka_map.listLayers() if lyr.name.upper() == "TEHSIL"), None)
            village_layer = next((lyr for lyr in village_map.listLayers() if lyr.name.upper() == "VILLAGE"), None)

            if not district_layer or not taluka_layer or not village_layer:
                arcpy.AddError("❌ Required layers not found.")
                continue
            ...
            village_layer.definitionQuery = taluka_query
            arcpy.management.SelectLayerByAttribute(village_layer, "NEW_SELECTION", taluka_query)
            village_extent = village_frame.getLayerExtent(village_layer, False, True)
            arcpy.management.SelectLayerByAttribute(village_layer, "CLEAR_SELECTION")
            apply_buffered_extent(village_frame, village_extent)

            village_names = sorted({row[0].strip() for row in arcpy.da.SearchCursor(village_layer, [village_field], taluka_query) if row[0]})
            arcpy.AddMessage(f"    📍 Villages found in {taluka_name}: {len(village_names)}")

            if not village_names:
                arcpy.AddWarning(f"⚠️ No Village found for: {taluka_name}")
                continue

            for village_index, village_name in enumerate(village_names, 1):
                village_total += 1
            
                # Skip if already exported
                safe_district = sanitize_filename(district_name)
                safe_taluka = sanitize_filename(taluka_name)
                safe_village = sanitize_filename(village_name)
                out_name = f"{safe_village}_{safe_taluka}_{safe_district}.pagx"
                ...
                arcpy.AddMessage(f"      📌 Processing Village: {village_index}. {village_name}")

                village_query = "{0} = '{1}' AND {2} = '{3}' AND {4} = '{5}'".format(
                    district_field, district_name.replace("'", "''"),
                    taluka_field, taluka_name.replace("'", "''"),
                    village_field, village_name.replace("'", "''")
                )

                village_fc = os.path.join(arcpy.env.scratchGDB, "highlight_village")
                if arcpy.Exists(village_fc):
                    arcpy.management.Delete(village_fc)
                arcpy.conversion.FeatureClassToFeatureClass(village_layer, arcpy.env.scratchGDB, "highlight_village", village_query)

                village_highlight_layer = village_map.addDataFromPath(village_fc)
                village_highlight_layer.name = "HighlightVillage"
                village_map.moveLayer(village_layer, village_highlight_layer, "BEFORE")
                apply_highlight_symbology(village_highlight_layer)
                
                safe_village = sanitize_filename(village_name)
                village_jpg = os.path.join(output_folder, f"{safe_village}.jpg")
                village_frame.exportToJPEG(village_jpg, resolution=150, jpeg_quality=70)

                village_pic = next((el for el in layout_template.listElements("PICTURE_ELEMENT") if el.name == "VillageMap"), None)
                if village_pic:
                    village_pic.sourceImage = village_jpg
                
                village_name_query = f"{village_field} = '{village_name}'"
                village_real_name = None
                with arcpy.da.SearchCursor(village_layer, [village_text_field], village_name_query) as cursor:
                    for row in cursor:
                        village_real_name = row[0].strip() if row[0] else None
                        break
                village_text = next((el for el in layout_template.listElements("TEXT_ELEMENT") if el.name == "VillageName"), None)
                if village_text and village_real_name:
                    village_text.text = f"{village_real_name}"
                    name_length = len(village_real_name)
                    if name_length > 27:
                        village_text.textSize = 7
                    elif name_length > 20:
                        village_text.textSize = 8
                    elif name_length > 15:
                        village_text.textSize = 10
                    else:
                        village_text.textSize = 12    
                ...
                if village_highlight_layer:
                    village_map.removeLayer(village_highlight_layer)
                if arcpy.Exists(village_fc):
                    arcpy.management.Delete(village_fc)

            village_layer.definitionQuery = ""  # 🧹 Reset after each taluka
    ...
    arcpy.AddMessage("\n📊 Summary:")
    arcpy.AddMessage(f"   🔹 Total villages processed: {village_total}")
    arcpy.AddMessage(f"   ⏩ Skipped (already exported): {village_skipped}")
    arcpy.AddMessage(f"   ✅ Newly exported: {village_exported}")

arcpy.AddMessage("\n🎯 All Villages layouts exported successfully. Total exported: {0}".format(village_exported))
